...
-- SCANNER FIX: antes o registry desistia PARA SEMPRE depois de 40 tentativas
-- (MAX_ATTEMPTS) e de 3 deep scans. Se o jogo demorasse a replicar o
-- Synchronizer (celular / join lento / update do jogo), o scanner ficava morto
-- a sessao inteira e nenhum brainrot aparecia. Agora nunca desiste: apos as
-- tentativas rapidas ele passa a tentar devagar (3s) e refaz deep scan
-- periodicamente. Use _G.XenSyncReset() para forcar uma redescoberta.
local SOFT_ATTEMPTS, RETRY_GAP = 40, 0.5
local SLOW_GAP = 3.0
local BOOT_T0, BOOT_BURST, BOOT_GAP = os.clock(), 3.0, 0.10
local DEEP_GAP, DEEP_GAP_SLOW = 1.5, 10.0

local function _retryGap()
    if (os.clock() - BOOT_T0) < BOOT_BURST then return BOOT_GAP end
    if _attempts >= SOFT_ATTEMPTS then return SLOW_GAP end
    return RETRY_GAP
end

-- A channel is a table with a CacheTable. Count them; the table holding the
-- most is the registry.
-- SCANNER FIX v15: um canal era reconhecido SO por rawget(v,"CacheTable").
-- Depois do update do jogo o registry passou a devolver canais via proxy
-- (__index), entao rawget dava nil, a contagem virava 0, o cache era
-- invalidado a cada frame e o scanner ficava sempre em 0 brainrots.
-- Agora aceitamos CacheTable por rawget OU por indexacao normal, e tambem
-- canais que expoem :Get (a forma antiga do modulo).
local function _looksChannel(v)
    if type(v) ~= "table" then return false end
    if type(rawget(v, "CacheTable")) == "table" then return true end
    local okC, ct = pcall(function() return v.CacheTable end)
    if okC and type(ct) == "table" then return true end
    local okG, g = pcall(function() return v.Get end)
    if okG and type(g) == "function" then return true end
    return false
end
...
local function _channelCount(t)
    ...
    local ok, hits = pcall(function()
        ...
        for _, v in next, t do
            n = n + 1
            if _looksChannel(v) then h = h + 1 end
...
-- Ultimo recurso: varre o garbage collector procurando a tabela de canais.
-- Funciona mesmo quando o modulo muda completamente de layout de upvalues.
local function _gcScan()
    local gc = getgc
    if type(gc) ~= "function" then return nil, 0, nil end
    local ok, best, bestN = pcall(function()
        local b, bn, seen = nil, 0, 0
        for _, v in ipairs(gc(true)) do
            if type(v) == "table" then
                seen = seen + 1
                local n = _channelCount(v)
                if n > bn then b, bn = v, n end
                if seen >= 60000 then break end
            end
        end
        return b, bn
    end)
    if ok and best then return best, bestN or _channelCount(best), "getgc" end
    return nil, 0, nil
end
...
local function _chans()
    -- revalida o cache: se a tabela latchada ficou sem nenhum canal (update do
    -- jogo / troca de servidor), descarta e redescobre em vez de devolver lixo.
    if _xchan then
        if _channelCount(_xchan) > 0 then return _xchan end
        _xchanLast = _xchan
        _xchan = nil
        _attempts = 0
        _deepScans = 0
        _G.XenSyncDiag = "cache invalidado (0 canais) - redescobrindo"
    end
    ...
    local _dgap = (_attempts >= SOFT_ATTEMPTS) and DEEP_GAP_SLOW or DEEP_GAP
    if not found and (os.clock() - _lastDeep) > _dgap then
        _lastDeep = os.clock()
        _deepScans = _deepScans + 1
        found, n, where = _deepScan(mod)
        if not found then found, n, where = _gcScan() end
    end
    ...
    if found then
        ...
        _G.XenSyncDiag = string.format("procurando canais... tentativa %d (deep %d)",
            _attempts, _deepScans)
        -- nunca ficar totalmente cego: devolve o ultimo registry conhecido
        if _xchanLast then return _xchanLast end
    end
    return _xchan
end

-- Forca uma redescoberta completa (util depois de um update do jogo ou se o
-- scanner travar em 0 brainrots): execute _G.XenSyncReset() no console.
_G.XenSyncReset = function()
    _xchan, _xchanLast, _mod = nil, nil, nil
    if _G.XenSyncMeowlClear then pcall(_G.XenSyncMeowlClear) end
    _attempts, _deepScans = 0, 0
    _lastTry, _lastDeep = 0, 0
    BOOT_T0 = os.clock()
    _G.XenSyncDiag = "reset manual"
    return _chans() ~= nil
end

-- ============================================================
-- MEOWL ENGINE (somente leitura, sem hook): NAO escrevemos nada no
-- modulo Synchronizer (o hook em GetAllChannels causava kick). Aqui
-- so PEDIMOS a tabela do canal com GetTableFromChannel quando o
-- descobridor original nao achou nada. Zero patch no jogo.
-- ============================================================
local _meowlCache = {}

local function _meowlChannel(idx)
    if idx == nil then return nil end
    if _meowlCache[idx] then return _meowlCache[idx] end
    local mod = _module()
    if not mod or type(mod.GetTableFromChannel) ~= "function" then return nil end
    local ok, ct = pcall(mod.GetTableFromChannel, mod, idx)
    if not ok or type(ct) ~= "table" then return nil end
    -- CacheTable exposto por rawget para que _G.sProp continue funcionando
    local proxy = setmetatable({ CacheTable = ct }, {
    ...
    _meowlCache[idx] = proxy
    return proxy
end

_G.XenSyncMeowlClear = function() _meowlCache = {} end
_G.XenSyncAll=function()return _chans()end
_G.XenSyncGet=function(idx)
if idx==nil then return nil end
-- 1) motor ORIGINAL primeiro (é o que o TP/clone espera)
local t=_chans()
if t then
...
-- 2) fallback Meowl (só quando o original falhou)
local okM,m=pcall(_meowlChannel,idx)
if okM and type(m)=="table" then return m end
return nil
end
...
do
    ...
    -- =====================================================================
    -- BYPASS NOVO: IDENTITY-SPOOF + TRAMPOLINE (metodo do Vstaaaaa.lua)
    -- ---------------------------------------------------------------------
    -- Em vez de adivinhar o hash pela estrutura da pasta Net, chamamos o
    -- PROPRIO resolver do jogo (NetMod:RemoteEvent/RemoteFunction) de dentro
    -- de um chunk com o env de um script legitimo do jogo e com thread
    -- identity 2. Assim o modulo devolve o remote HASHEADO real em vez do
    -- alias morto, sem hook nenhum (nada pra canary do anti-cheat pegar).
    -- Os upvalues do resolver que apontam pra scripts sao trocados pelo
    -- script-mascara durante a chamada e restaurados logo depois.
    -- =====================================================================
    local _spoofResolve
    do
        local mask
        do
            ...
            if ctrl then
                for _, v in ipairs(ctrl:GetChildren()) do
                    if v:IsA("LuaSourceContainer") and v.Name ~= "PlotController" then
                        mask = v; break
                    end
                end
            end
            if not mask then
                ...
                if datas then
                    for _, v in ipairs(datas:GetChildren()) do
                        if v:IsA("LuaSourceContainer") then mask = v; break end
                    end
                end
            end
        end
        ...
        local function spoof(fn, ...)
            if not (mask and gti and sti and loadstring and getrenv) then return nil end
            if not trampoline then
                local ok = pcall(function()
                    local src = "return function(e,f,...) setfenv(0,e) local function c(...) return f(...) end local function b(...) return c(...) end local function a(...) return b(...) end return a(...) end"
                    local chunk = loadstring(src, "=" .. mask:GetFullName())
                    local ok2, env = pcall(getsenv, mask)
                    local renv = getrenv()
                    setfenv(chunk, (ok2 and type(env) == "table") and env
                        or setmetatable({script = mask, _G = {}, shared = {}}, {__index = renv, __newindex = renv}))
                    trampoline = chunk()
                end)
                if not ok or not trampoline then return nil end
            end

            local gu = getupvalues or (debug and debug.getupvalues)
            local su = setupvalue  or (debug and debug.setupvalue)
            local restored = {}
            if gu and su then
                local ok2, uvs = pcall(gu, fn)
                if ok2 and type(uvs) == "table" then
                    for i, v in pairs(uvs) do
                        if typeof(v) == "Instance" and v:IsA("LuaSourceContainer") then
                            if pcall(su, fn, i, mask) then restored[i] = v end
                        end
                    end
                end
            end

            local prev = gti(); sti(2)
            local ok3, result = coroutine.resume(coroutine.create(trampoline), getrenv(), fn, ...)
            sti(prev)
            for i, v in pairs(restored) do pcall(su, fn, i, v) end
            return ok3 and result or nil
        end

        local _netMod
        local function getNetMod()
            if type(_netMod) == "table" then return _netMod end
            local ok, m = pcall(require, netFolder)
            if ok and type(m) == "table" then _netMod = m end
            return _netMod
        end

        local prefixes = { RemoteEvent = "RE", RemoteFunction = "RF", UnreliableRemoteEvent = "URE" }
        local _spoofCache = {}

        -- Resolve por identity-spoof. Retorna o Instance hasheado ou nil.
        _spoofResolve = function(name, kind)
            kind = (kind == "RemoteFunction" and "RemoteFunction")
                or (kind == "UnreliableRemoteEvent" and "UnreliableRemoteEvent")
                or "RemoteEvent"
            local ck = kind .. "/" .. tostring(name)
            local c = _spoofCache[ck]
            if c and c.Parent then return c end
            local NetMod = getNetMod()
            if not NetMod then return nil end
            local logical  = name:match("^R[EF]/(.+)$") or name:match("^URE/(.+)$") or name
            local resolver = rawget(NetMod, kind)
            if type(resolver) ~= "function" then return nil end
            local r = spoof(resolver, NetMod, logical)
            if typeof(r) ~= "Instance" then return nil end
            -- alias morto em texto puro -> rejeita
            if r.Name == (prefixes[kind] or "RE") .. "/" .. logical then return nil end
            _spoofCache[ck] = r
            return r
        end
        _G.SXESpoofResolve = _spoofResolve
    end

    -- Resolve the live hashed remote for a plaintext name (hook-free).
    -- RESOLVER 1:1 DO jp.txt: dual-hash -> adjacencia -> _G.Net (hash real do
    -- jogo, F1/F2). O identity-spoof foi REMOVIDO daqui: era ele que fazia o
    -- anti-cheat expulsar, e o jp nunca precisou dele.
    if _G.SXEAllowSpoof == nil then _G.SXEAllowSpoof = false end
    -- Resolve the live hashed remote for a plaintext name (hook-free).
    ...
    task.spawn(function()
        -- WARM-UP igual ao jp: requer o modulo Net (poe as funcoes de hash no
        -- heap para o getgc achar) e ja resolve o UseItem antes do primeiro TP.
        pcall(function()
            local pkgs = game:GetService("ReplicatedStorage"):WaitForChild("Packages", 30)
            if pkgs then require(pkgs:WaitForChild("Net", 30)) end
        end)
        local t0 = os.clock()
        while os.clock() - t0 < 60 do
            if resolveHashed("UseItem", "RemoteEvent") then
                warn("[SXE] UseItem resolvido: " .. tostring(resolveHashed("UseItem", "RemoteEvent")))
                return
            end
    ...
    -- ============================================================
    --  SIMULACAO REAL DO GRAPPLE HOOK
    --  Antes de mandar o remote, faz tudo que um jogador de verdade faz:
    --  equipa a ferramenta pelo Humanoid, vira o personagem pro destino,
    --  aponta o mouse no ponto da tela e clica (Activate + clique virtual).
    --  Isso deixa o uso indistinguivel de um clique legitimo e resolve o
    --  caso do "gancho fica na mao e nao acontece nada".
    -- ============================================================
    local GRAPPLE_NAMES = { "Grapple Hook", "GrappleHook", "Grapple", "Hook", "Gancho" }

    _G.SXEFindGrappleTool = function()
        ...
        local function pick(parent)
            ...
            for _, t in ipairs(parent:GetChildren()) do
                if t:IsA("Tool") and (t.Name:lower():find("grapple") or t.Name:lower():find("hook") or t.Name:lower():find("gancho")) then
                    return t
                end
            end
            return nil
    ...
    _G.SXEEquipGrapple = function()
        local plr = game:GetService("Players").LocalPlayer
        local char = plr and plr.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local tool = _G.SXEFindGrappleTool()
        if not tool then return nil end
        if tool.Parent ~= char then
            if hum then pcall(function() hum:EquipTool(tool) end) end
            local t0 = os.clock()
            while tool.Parent ~= char and os.clock() - t0 < 0.5 do task.wait(0.03) end
        end
    ...
    -- ============================================================
        ...
        -- ============================================================
        --  SIMULACAO COMPLETA (metodo message(1).txt)
        --  - Muta beam/som
        --  - Cria alvo falso na frente
        --  - Escreve PlayerMouse.Hit/Target no alvo
        --  - Ativa repetidamente ate o FlightPower aparecer
        --  - Zera o MaxForce do FlightPower (sem puxao visivel)
        --  - Restaura e limpa tudo
        -- ============================================================
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then return false end
        local RSvc = game:GetService("RunService")
        
        -- 1. Muta o beam e os sons
        pcall(function()
            ...
            if beam and beam:IsA("Beam") then
                ...
                beam.Width1 = 0
                local beamConn
                beamConn = beam:GetPropertyChangedSignal("Width0"):Connect(function()
                    beam.Width0 = 0
                    beam.Width1 = 0
                end)
                task.delay(2, function() if beamConn then beamConn:Disconnect() end end)
            end
            for _, snd in ipairs(tool:GetDescendants()) do
                if snd:IsA("Sound") then snd.Volume = 0 end
            end
        end)
        
        -- 2. Prepara o shield pro FlightPower
        local fpCreated = false
        local fpZeroed = false
        local shield = hrp.ChildAdded:Connect(function(child)
            if child:IsA("BodyVelocity") and child.Name == "FlightPower" then
                fpCreated = true
                pcall(function() child.MaxForce = Vector3.zero end)
                fpZeroed = true
            end
        end)
        
        -- 3. Cria alvo falso na frente (11 studs)
        ...
        fakeTarget.CFrame = hrp.CFrame * CFrame.new(0, 0, -11)
        fakeTarget.Parent = workspace
        
        -- 4. Usa PlayerMouse para apontar pro alvo falso
        local ok = false
        ...
        local mira = fakeTarget.CFrame
        local antes = {}
        for i, t in ipairs(refs) do
            antes[i] = { rawget(t, "Hit"), rawget(t, "Target") }
            pcall(function() t.Hit = mira; t.Target = fakeTarget end)
        end
        
        -- 5. Ativa repetidamente ate o FlightPower aparecer (max 4s)
        local _gTime = os.clock()
        while not fpCreated and os.clock() - _gTime < 4.0 do
            if tool and tool.Parent then
                ok = pcall(function() tool:Activate() end)
            end
            task.wait(0.05)
            -- Segura o estado por ate 0.1s no frame corrente
            local _tWait = os.clock()
            while not fpCreated and os.clock() - _tWait < 0.1 do
        ...
        -- 6. Restaura PlayerMouse e limpa
        for i, t in ipairs(refs) do
            local a = antes[i]
            pcall(function() t.Hit = a[1]; t.Target = a[2] end)
        end
        if fakeTarget then fakeTarget:Destroy() end
        if shield then shield:Disconnect() end
        
        return ok
    end

    -- ============================================================
    --  O METODO COMPLETO: resolve remote, equipa, mira, dispara
    -- ============================================================
    _G.SXEFireGrapple2 = function(value, exato)
        ...
        -- Sao TRES argumentos. O hub mandava so o primeiro e o servidor
        -- nao tinha como saber pra onde puxar. O 3o e uma BasePart de
        -- verdade: e por ela que o servidor confere se existe algo.
        ...
        -- Removed Stealing check: the script is always stealing, this blocked the grapple

        -- EQUIPA A TOOL SE PRECISO (antes de qualquer coisa, sem yield
        -- entre escrever a mira e chamar Activate)
        ...
        -- MIRA EXATA (exato = true): usa o ponto como veio, sem busca
        ...
        -- O item so dispara com o alvo entre 10 e 50 studs.
        -- Comeca 3 studs acima da cabeca e desce 60, ficando sob a cobertura.
        -- Testa todos os passos e fica com o de MENOR distancia.
        ...
        -- sem chao: raio reto na direcao pedida
        if not pos then
        ...
        -- Varre o heap atras de toda tabela com {Hit=CFrame, Target=...}
        -- e escreve em todas. A do jogo esta entre elas.
        return _G.__SXEGrappleDisparar(tool, pos, alvo, char, RSv)
    end
    ...
    -- ============================================================
    --  SXECicloTP: The full TP cycle
    --  Grapple toward base -> wait -> fire TP
    -- ============================================================
    _G.SXECicloTP = function(esperaMax)
        ...
        -- Wait for a target in cache before firing hook
        ...
        -- Find target base position for streaming + grapple direction
        local destino
        do
        ...
        -- Request streaming of destination
        if workspace.StreamingEnabled and destino then
            pcall(function() eu:RequestStreamAroundAsync(destino) end)
        end

        -- Fire grapple toward base (jp.txt: fires with default float, no Vector3)
        _G.__SXEGrappleDestino = destino
        local ok = false
        if destino and _G.SXEFireGrapple2 then
            pcall(function() ok = _G.SXEFireGrapple2(destino) and true or false end)
        end

        -- Wait between hook and TP (can't be zero - Deferred signal)
        local ESPERA_HOOK_TP = 0.12
        task.wait(ESPERA_HOOK_TP)

        -- Chama o motor Clone-TP CRU (nunca o dispatcher), senao o dispatcher
        -- voltaria pro grapple e o ciclo entraria em recursao infinita.
        local _tp = _G.SXECloneTPRaw or _G.SXETPv1 or _G.SXEStartSideTP or _G.SXE_ExecuteManualTP
        if _tp then task.spawn(function() pcall(_tp) end) end
...
local function LazyInit(name, fn)
    table.insert(_G.__SXELazyQ, { name = name, fn = fn })
end
...
local old = playerGui:FindFirstChild("STLHub_V3"); if old then old:Destroy() end
local gui_sg = Instance.new("ScreenGui"); gui_sg.Name = "STLHub_V3"; gui_sg.ResetOnSpawn = false; gui_sg.IgnoreGuiInset = true; gui_sg.DisplayOrder = 9999999; gui_sg.Parent = playerGui
...
Themes = {
    ...
    Dark = {
        Background=Color3.fromRGB(0,0,0), MainBackground=Color3.fromRGB(5,5,5),
        Panel=Color3.fromRGB(10,10,12), Row=Color3.fromRGB(15,15,18), RowHover=Color3.fromRGB(25,25,30),
        Accent=Color3.fromRGB(0,120,255), AccentLight=Color3.fromRGB(50,150,255),
        Green=Color3.fromRGB(0,200,100), Red=Color3.fromRGB(235,70,70), Red2=Color3.fromRGB(200,55,55),
        Text=Color3.fromRGB(255,255,255), Dim=Color3.fromRGB(150,150,150), Stroke=Color3.fromRGB(40,40,50),
        SoftButton=Color3.fromRGB(20,20,30), SoftButtonHover=Color3.fromRGB(30,30,45),
        SoftAccent=Color3.fromRGB(35,35,50), SoftAccentHover=Color3.fromRGB(45,45,60),
        ToggleOff=Color3.fromRGB(50,50,60), ToggleOff2=Color3.fromRGB(15,15,20),
        InputBg=Color3.fromRGB(18,18,22), SliderBg=Color3.fromRGB(30,30,45),
        BlacklistHover=Color3.fromRGB(90,45,45), BlacklistLeave=Color3.fromRGB(30,30,40),
    }
}
Theme = {}
...
function applyTheme(themeName)
    ...
    local function updateBottomBarTheme()
        ...
        for _, child in ipairs(bottomBar:GetChildren()) do
            ...
            elseif child:IsA("TextLabel") then
                if child.Text == "|" or child.Text == "STL HUB" then
                    child.TextColor3 = toTheme.AccentLight
                elseif child.Text == "By: STL HUB" then
                    child.TextColor3 = toTheme.Dim
                elseif child.Text == "STL HUB v2" then
                    child.TextColor3 = toTheme.AccentLight
                end
            elseif child:IsA("Frame") then
    ...
    pcall(function()
        local sg = playerGui:FindFirstChild("STLHub_V3")
    ...
    -- text invisible on sxehub's white accents -- so only the slider knob stays forced white.
...
actionConfig = {
    ...
    ["Anti Ragdoll"]=false,["Infinite Jump"]=false,["Float"]=false,["Carpet Speed"]=false,["Teleport"]=true,
    ["Teleport (T)"]=true,["Server Hop"]=true,["Auto Close GUI"]=false,
}
...
-- SYNCHRONIZER RelateChannels DETECTION NEUTER (ported from "synchronizer bypass.txt" / sxehub) --
...
do
    ...
    if okInline and landed then
        _G.__SXE_RelateKilled = true
    else
        task.spawn(function()
            for _ = 1, 20 do
                task.wait(0.4)
                local ok, done = pcall(_sxeNeuterRelate)
                if ok and done then _G.__SXE_RelateKilled = true break end
            end
        end)
    end
end
...
local function hookHeartbeatRemote()
    -- DESATIVADO: qualquer hook em FireServer/__namecall e detectado pelo anti-cheat.
    do return end
...
-- RYXX INSTANT RESET (ported from Ryxx)
-- Phase 1: teleport the character far away to force-respawn it, with multi-attempt retry.
local _RYXX_RESET_CFRAME = CFrame.new(1000003.56, 999999.69, 8.17)
local function ryxxPhase1()
    task.spawn(function()
        ...
        if currentRoot and currentHum and currentHum.Health > 0 then
            ...
            for i = 1, 50 do
                ...
                currentRoot = currentChar and currentChar:FindFirstChild("HumanoidRootPart")
                currentHum = currentChar and currentChar:FindFirstChildOfClass("Humanoid")
                if not currentRoot or not currentHum or currentHum.Health <= 0 then killed = true break end
                currentRoot.CFrame = _RYXX_RESET_CFRAME
...
-- Phase 2: fire the reset remote (balloon payload) until the character actually changes.
...
-- HOOK-FREE: no hookfunction (trips anti-cheat's remote-fire canary on load).
-- Scan ReplicatedStorage for the "RE/..." remote instead, or set _G.SXE_RESET_REMOTE before running.
if typeof(_G.SXE_RESET_REMOTE) == "Instance" then cursedResetRemote = _G.SXE_RESET_REMOTE end

-- RESET FIX v15: antes pegava o PRIMEIRO RemoteEvent "RE/..." encontrado, que
-- muitas vezes nao era o de reset (ordem de GetDescendants muda) -> reset falhava
-- ou demorava. Agora pontuamos os candidatos por nome e ignoramos remotes de
-- staff/admin. O melhor candidato fica cacheado em _G.SXE_RESET_REMOTE.
local function _resetScore(name)
    local n = name:lower()
    for _, bad in ipairs({"adminreset","staffreset","modreset"}) do
        if n:find(bad, 1, true) then return -1 end
    end
    local sc = 0
    if n:find("reset", 1, true) then sc = sc + 50 end
    if n:find("respawn", 1, true) then sc = sc + 45 end
    if n:find("balloon", 1, true) then sc = sc + 8 end
    if n:find("die", 1, true) or n:find("kill", 1, true) then sc = sc + 20 end
    if n:find("character", 1, true) then sc = sc + 10 end
    if name:sub(1,3) == "RE/" then sc = sc + 5 end
    return sc
end
...
local function findResetRemote()
    ...
    for _, desc in ipairs(ReplicatedStorage:GetDescendants()) do
        if desc:IsA("RemoteEvent") then
            local sc = _resetScore(desc.Name)
            if sc > bestScore then best, bestScore = desc, sc end
        end
    end
    if not best then
        -- fallback: comportamento antigo (primeiro remote "RE/..."), que e o
        -- canal generico usado pelo payload balloon neste jogo.
        for _, desc in ipairs(ReplicatedStorage:GetDescendants()) do
            if desc:IsA("RemoteEvent") and desc.Name:sub(1,3) == "RE/" then best = desc; break end
        end
    end
    if best then
        cursedResetRemote = best
        _G.SXE_RESET_REMOTE = best
    end
end
task.spawn(function() findResetRemote(); task.wait(2); findResetRemote() end)

local function fireReset()
    findResetRemote()
    if not cursedResetRemote then return false end
    local ok, err = pcall(function() cursedResetRemote:FireServer(RESET_GUID, LocalPlayer, "balloon") end)
    return ok
end
...
local function instantReset()
    ...
    -- Unequip tools before resetting
    pcall(function()
        if character and humanoid then
            for _, tool in ipairs(character:GetChildren()) do
                if tool:IsA("Tool") and tool.Parent == character then humanoid:UnequipTools(); break end
            end
        end
    end)

    -- Phase 1: Ryxx teleport-away force-respawn
    ryxxPhase1()

    -- Already dead: one fire is enough.
    ...
    -- Phase 2: fire the reset remote with balloon payload until character changes.
    ...
    task.spawn(function()
        local attempts = 0
        -- spam mais rapido (0.03s) e por mais tempo, com fallback local se em
        -- ~0.35s o personagem ainda nao tiver morrido/trocado.
        for _ = 1, 16 do
            if resetDetected then break end
            attempts = attempts + 1
            fireReset()
            if attempts == 1 and not cursedResetRemote then
                pcall(function() if humanoid then humanoid.Health = 0 end end)
            end
            if attempts == 10 and not resetDetected then
                pcall(function() if humanoid then humanoid.Health = 0 end end)
        ...
        -- libera o cooldown assim que o novo personagem aparecer (permite spam)
        if resetDetected then
            local t0 = os.clock()
            while not (lp.Character and lp.Character:FindFirstChildOfClass("Humanoid")) and os.clock() - t0 < 3 do
                task.wait(0.05)
            end
        end
        instaResetCooldown = false
...
-- CARPET SPEED  (corre rapido com o tapete na mao)
-- Velocidade ajustavel via Config.TpSettings.CarpetSpeedValue (padrao 140).
CarpetState={enabled=false,conn=nil}
local CARPET_TOOL_NAMES = { "Flying Carpet", "Carpet", "Magic Carpet", "Cloud", "Witch's Broom", "Cupid's Wings", "Santa's Sleigh", "Waverider" }
local function _carpetSpeedValue()
    local v = tonumber(Config and Config.TpSettings and Config.TpSettings.CarpetSpeedValue) or tonumber(_G.SXECarpetSpeedValue) or 140
    if v < 16 then v = 16 elseif v > 400 then v = 400 end
    return v
end
local function _findCarpetTool()
    local c = player.Character
    local bp = player:FindFirstChild("Backpack")
    local pref = (Config and Config.TpSettings and Config.TpSettings.Tool) or "Flying Carpet"
    local names = { pref }
    for _, n in ipairs(CARPET_TOOL_NAMES) do if n ~= pref then names[#names+1] = n end end
    for _, n in ipairs(names) do
        local t = (c and c:FindFirstChild(n)) or (bp and bp:FindFirstChild(n))
        if t and t:IsA("Tool") then return t, n end
    end
    return nil
end
local function setCarpetSpeed(en) CarpetState.enabled=en; setToggle("Carpet Speed",en)
    if Config then Config.CarpetSpeed = en end
    ...
    CarpetState.conn=RunService.Heartbeat:Connect(function()
        ...
        local c=player.Character; if not c then return end
        -- Nao aplica o boost durante steal/TP: a diferenca de velocidade nesse
        -- momento e exatamente o que o anti-cheat procura.
        ...
        local tool, tname = _findCarpetTool()
        if not tool then return end
        if tool.Parent ~= c then pcall(function() hum:EquipTool(tool) end); return end
        local spd = _carpetSpeedValue()
...
_G.setCarpetSpeedValue = function(v)
    if Config and Config.TpSettings then Config.TpSettings.CarpetSpeedValue = tonumber(v) or 140 end
    _G.SXECarpetSpeedValue = tonumber(v) or 140
end

-- INFINITE JUMP  (LOGICA DO PC, IGUAL NO CELULAR)
-- Agora usa exatamente a logica classica do PC: UserInputService.JumpRequest ->
-- Humanoid:ChangeState(Jumping). O JumpRequest dispara no Space (PC), no botao
-- de pulo do touch (celular) e no gamepad, entao o comportamento e identico nos
-- tres. Removido o empurrao de AssemblyLinearVelocity e o loop de Heartbeat que
-- forcava pulos todo frame -- era isso que gerava velocidade impossivel e
-- pulsos constantes, o padrao mais facil de detectar.
InfJumpState={enabled=false,conn=nil,conns={},lastJump=0}
local function _infJumpPulse()
    if not InfJumpState.enabled then return end
    local c=player.Character; if not c then return end
    local hum=c:FindFirstChildOfClass("Humanoid")
    if not hum or hum.Health<=0 then return end
    if hum.Sit then return end
    local now=tick(); if now-InfJumpState.lastJump<0.12 then return end
    InfJumpState.lastJump=now
...
local function setInfiniteJump(en) InfJumpState.enabled=en; Config.InfiniteJump=en; Config.TpSettings.InfiniteJump=en; setToggle("Infinite Jump",en); saveConfig()
    for _, cn in ipairs(InfJumpState.conns) do pcall(function() cn:Disconnect() end) end
    InfJumpState.conns = {}
    if InfJumpState.conn then InfJumpState.conn:Disconnect(); InfJumpState.conn=nil end
    if not en then return end
    InfJumpState.conns[#InfJumpState.conns+1] = UIS.JumpRequest:Connect(_infJumpPulse)
end
...
-- AUTO RESET BALLOON v16: antes so varria a GUI a cada 1s, ou seja o reset podia
-- sair quase 1s depois do balao aparecer. Agora reage por evento (DescendantAdded
-- + mudanca de Text) e mantem um poll rapido de 0.15s como rede de seguranca.
-- Usa o mesmo executeReset -> instantReset melhorado.
local function _isBalloonText(txt)
    return type(txt)=="string" and (string.find(txt,'ran "balloon" on you',1,true) or string.find(txt,"ran 'balloon' on you",1,true))
end
local _balloonLast=0
local function _balloonTrigger()
    if not Config.AutoResetBalloon then return end
    if os.clock()-_balloonLast < 1.5 then return end
    _balloonLast=os.clock()
    executeReset(true)
end
local function _watchBalloonLabel(g)
    if not (g:IsA("TextLabel") or g:IsA("TextButton")) then return end
    if _isBalloonText(g.Text) then _balloonTrigger() end
    g:GetPropertyChangedSignal("Text"):Connect(function() if _isBalloonText(g.Text) then _balloonTrigger() end end)
end
task.spawn(function()
    for _,g in ipairs(playerGui:GetDescendants()) do pcall(_watchBalloonLabel,g) end
    playerGui.DescendantAdded:Connect(function(g) pcall(_watchBalloonLabel,g) end)
end)
task.spawn(function() while true do task.wait(0.15); if not Config.AutoResetBalloon then continue end
    for _,g in ipairs(playerGui:GetDescendants()) do local txt=(g:IsA("TextLabel") or g:IsA("TextButton")) and g.Text
        if _isBalloonText(txt) then _balloonTrigger(); break end end end end)
...
local function fireGrapple()
    ...
    -- jp.txt: wait for the equip to actually register
    local _te = os.clock()
    while not (LP.Character and LP.Character:FindFirstChild("Grapple Hook"))
          and os.clock() - _te < 0.5 do
        RunService.Heartbeat:Wait()
    end
    if not (LP.Character and LP.Character:FindFirstChild("Grapple Hook")) then return end
...
local function carpetEngage()
    ...
    if g then
        ...
        -- jp.txt: fire with default float value (0.33) - the arg is a FLOAT not Vector3
        if _G.SXEFireGrapple2 then pcall(_G.SXEFireGrapple2) end
...
function runAutoSnipe()
    ...
    -- ALWAYS use sxehub's velocity TP (doVelocityTP) -- the old carpet-glide/grapple path
...
-- sxehub tweak: Highlights are NOT disabled, so hub ESP outlines keep working while boost is on.
;(function()
...
decoratePanel = function(f)
    ...
    local hl=Instance.new("Frame"); hl.Name="GlassRim"; hl.Size=UDim2.new(1,-16,0,1); hl.Position=UDim2.new(0,8,0,1); hl.BackgroundColor3=Color3.fromRGB(243,243,243); hl.BackgroundTransparency=0.5; hl.BorderSizePixel=0; hl.ZIndex=6; hl.Parent=f
...
function makeHeader(f,t,isMain) local h=Instance.new("Frame"); h.Size=UDim2.new(1,0,0,42); h.BackgroundTransparency=1; h.Parent=f
    ...
    if isMain then local l=Instance.new("TextLabel"); l.Size=UDim2.new(1,-50,0,24); l.Position=UDim2.new(0,13,0,8); l.BackgroundTransparency=1; l.Text=parts[1] or "STL HUB"; l.TextColor3=Theme.Text; l.Font=Enum.Font.GothamBlack; l.TextSize=16; l.TextXAlignment=Enum.TextXAlignment.Left; l.Parent=h
    else local l=Instance.new("TextLabel"); l.Size=UDim2.new(1,-58,0,16); l.Position=UDim2.new(0,12,0,7); l.BackgroundTransparency=1; l.Text=parts[1] or "STL HUB"; l.TextColor3=Theme.Text; l.Font=Enum.Font.GothamBlack; l.TextSize=12; l.TextXAlignment=Enum.TextXAlignment.Center; l.Parent=h
...
-- CREATE PANELS
main,mainBody=makeMainPanel("STL HUB",UDim2.new(0,560,0,560),UDim2.new(0.5,-280,0.5,-290))
...
function rebuildActions()
    ...
    if actionConfig["Teleport (T)"] then makeQuickButton(panels["ActionsBody"],"Teleport (T)",function() task.spawn(runAutoSnipe) end) end
    ...
    if actionConfig["Server Hop"] then makeQuickButton(panels["ActionsBody"],"Server Hop",function()
        task.spawn(function()
            local ok, servers = pcall(function()
                return game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Net"):WaitForChild("Net").GetServers()
            end)
            if not ok or not servers or #servers == 0 then
                -- Fallback: join a random server of the same game
                pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, player) end)
            else
                local randomServer = servers[math.random(1, #servers)]
                pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, randomServer, player) end)
            end
        end)
    end) end
    if actionConfig["Auto Close GUI"] then makeSyncStateRow(panels["ActionsBody"],"Auto Close GUI:","Auto Close GUI",function(on) if _G.setAutoCloseGUI then _G.setAutoCloseGUI(on) end end) end
    ...
    if actionConfig["Reset (X)"] then makeQuickButton(panels["ActionsBody"],"Reset (X)",function() executeReset() end,Theme.Red2) end
...
function spamBaseOwner()
    ...
    if ok and Synchronizer then
        ...
        local ch = (_G.SXE_GetPlotChannel and _G.SXE_GetPlotChannel(nearestPlot.Name)) or nil
        if ch then
            local owner = ch:Get("Owner")
            if owner then
                if (typeof(owner) == "Instance") and owner:IsA("Player") then
...
do
    ...
    print(("[SXE thumbs] httpreq=%s  getcustomasset=%s  writefile=%s  isfile=%s"):format(
    ...
    function BrainrotImg.get(name)
        ...
        task.spawn(function()
            ...
            local paths = { "sxe_brainrots/" .. fileKey .. ".png", "sxe_br_" .. fileKey .. ".png" }
            local asset, reason
            ...
            if asset then
                ...
                if not warned then
                    warned = true
                    warn("[SXE thumbs] '" .. key .. "' -> " .. (reason or "unknown")
...
_G.SXE_GetBrainrotImg = BrainrotImg.get   -- shared with the priority-alert banner
...
LazyInit("Admin Panel UI", function() -- ADMIN PANEL UI SCOPE
    ...
    ;(function()
        ...
        local function refreshBL()
            ...
            for _,nm in ipairs(Config.apBlacklist or {}) do
...
local tabs={"Keybind","Auto TP","ESP","UI","Misc","Priority","Perform"}
...
function loadTab(tabName)
    ...
    elseif tabName=="Auto TP" then
        ...
        makeMainToggle(mainBody,"Grabble TP",Config.TpSettings.GrabbleTP,function(on)
            ...
            if on then
                -- aquece o resolver do RE/UseItem para o 1o TP nao falhar
                task.spawn(function() pcall(function() return _G.SXEGrappleReady and _G.SXEGrappleReady() end) end)
                if _G.SXESetCarpetSpeed then
                    pcall(_G.SXESetCarpetSpeed, tonumber(Config.TpSettings.GrabbleTPSpeed) or 230)
                end
            end
            ShowNotification("GRABBLE TP", on and "ENABLED" or "DISABLED")
        end)
...
-- CORNER BALL (left side middle with STL logo - opens main menu)
local cornerBall=Instance.new("ImageButton"); cornerBall.Size=UDim2.new(0,110,0,110); cornerBall.Position=UDim2.new(0,15,0.5,-55); cornerBall.BackgroundTransparency=1; cornerBall.BorderSizePixel=0; cornerBall.AutoButtonColor=false; cornerBall.ZIndex=999999; cornerBall.Parent=gui
local cornerBallImg=Instance.new("ImageLabel"); cornerBallImg.Size=UDim2.new(1,-6,1,-6); cornerBallImg.Position=UDim2.new(0,3,0,3); cornerBallImg.BackgroundTransparency=1; cornerBallImg.ScaleType=Enum.ScaleType.Crop; cornerBallImg.Image="rbxassetid://103242575132029"; cornerBallImg.ImageColor3=Color3.fromRGB(255,255,255); cornerBallImg.ClipsDescendants=true; cornerBallImg.Parent=cornerBall
local cornerBallImgCorner=Instance.new("UICorner"); cornerBallImgCorner.CornerRadius=UDim.new(1,0); cornerBallImgCorner.Parent=cornerBallImg
cornerBall.ClipsDescendants=true
local cornerBallStroke=Instance.new("UIStroke"); cornerBallStroke.Color=Theme.Accent; cornerBallStroke.Thickness=2; cornerBallStroke.Transparency=0.3; cornerBallStroke.Parent=cornerBall
local cornerBallCorner=Instance.new("UICorner"); cornerBallCorner.CornerRadius=UDim.new(1,0); cornerBallCorner.Parent=cornerBall
cornerBall.MouseButton1Click:Connect(function()
    if main.Visible then closeAnim(main) else openAnim(main) end
end)
-- BOTTOM BAR
...
_G.updateLogoImage = function(isDark)
    ic.Image = "rbxassetid://103242575132029"
    ic.ImageColor3 = Color3.fromRGB(255, 255, 255)
    iw.BackgroundTransparency = 0.2
    iw.BackgroundColor3 = Theme.SoftAccent
end
_G.updateLogoImage(Config and Config.DarkMode or false)
local lg=Instance.new("TextLabel"); lg.Size=UDim2.new(0,150,0,26); lg.Position=UDim2.new(0,54,0,5); lg.BackgroundTransparency=1; lg.Text="STL HUB"; lg.TextColor3=Theme.AccentLight; lg.Font=Enum.Font.GothamBlack; lg.TextSize=19; lg.TextXAlignment=Enum.TextXAlignment.Left; lg.Parent=bottomBar
...
local dc=Instance.new("TextLabel"); dc.Size=UDim2.new(0,210,0,26); dc.Position=UDim2.new(0,230,0,5); dc.BackgroundTransparency=1; dc.Text="STL HUB v2"; dc.TextColor3=Theme.AccentLight; dc.Font=Enum.Font.GothamBold; dc.TextSize=16; dc.TextXAlignment=Enum.TextXAlignment.Left; dc.Parent=bottomBar
local sb=Instance.new("TextLabel"); sb.Size=UDim2.new(0,290,0,14); sb.Position=UDim2.new(0,55,0,30); sb.BackgroundTransparency=1; sb.Text="By: STL HUB"; sb.TextColor3=Theme.Dim; sb.Font=Enum.Font.GothamSemibold; sb.TextSize=8; sb.TextXAlignment=Enum.TextXAlignment.Left; sb.Parent=bottomBar
...
do
    ...
    -- jp.txt method: equip grapple, WAIT for equip to register, then fire via firesignal
    local function fireGrapple()
        if not NetModule then loadNet() end
        if not NetModule then return end
        local char = LP.Character
        if not char then return end
        -- Get the grapple tool from char or backpack
        local tool = char:FindFirstChild("Grapple Hook")
        if not tool then
            local bp = LP:FindFirstChild("Backpack")
        ...
        -- jp.txt: wait for the equip to actually register (tool becomes child of char)
        local _te = os.clock()
        while not (LP.Character and LP.Character:FindFirstChild("Grapple Hook"))
              and os.clock() - _te < 0.5 do
            RunService.Heartbeat:Wait()
        ...
        if not (LP.Character and LP.Character:FindFirstChild("Grapple Hook")) then return end
        tool = LP.Character:FindFirstChild("Grapple Hook")
        if not tool then return end
        -- Fire the live hashed RE/UseItem with a far-grapple float
        if _G.SXEFireGrapple2 then pcall(_G.SXEFireGrapple2) end
    end
...
local function carpetEngage()
        ...
        if LP.Character and LP.Character:FindFirstChild("Grapple Hook") then
            ...
            -- jp.txt method: fire grapple and wait 0.22s for projectile to register and pull
    ...
    local function doVelocityTP()
        ...
        -- SabcomLeaks method: scan with pcall for safety
        ...
        -- SabcomLeaks method: Conveyor pets are EXCLUDED from auto-selection (they teleport wrong)
        -- Respect manual selection, then fall back to priority/mode
        local pet
        if manuallySelectedUID then
        ...
        -- SabcomLeaks method: _f2FromF1 disabled
        local _f2FromF1 = false
        local coordTable = adjY > UPPER_Y_THRESHOLD and UPPER or LOWER

        -- jp.txt method: Fire the grapple THE INSTANT the scan resolves a target --
        -- equips the Grapple Hook and fires it immediately so the pull starts right away
        ...
        if pet.conveyor then
            ...
            -- SabcomLeaks method: TRAVAMENTO detection + vertical speed limit
            local _t0 = os.clock()
            ...
            while os.clock() - _t0 < 8 do
                ...
                -- TRAVAMENTO: sem isto, empurrava contra a parede pelos 8s inteiros
                if mag > _ultDist - 0.05 then _parado = _parado + 1 else _parado = 0 end
                _ultDist = mag
                if _parado >= 25 then break end
                equipCarpet()
                -- LIMITE VERTICAL (subida em velocidade cheia e a anomalia que o servidor mais detecta)
        ...
        if petPos.Y <= 8.9 and isPlotUnlocked(pet.plot) then
            ...
            -- SabcomLeaks: no F2F1 jump spam, just clean return
            if hrp and hrp.Parent then
                hrp.AssemblyLinearVelocity = Vector3.zero
                hrp.AssemblyAngularVelocity = Vector3.zero
            end
        ...
        if _cloneOk then
            ...
            -- SabcomLeaks: always call goToBrainrot if clone succeeded (no _forceGo)
    ...
    -- Referencia CRUA do motor Clone-TP. O dispatcher (_fxTpDispatch) sobrescreve
    -- _G.SXEStartSideTP la no fim do arquivo, entao guardamos o motor puro aqui
    -- para o Grabble TP poder chama-lo sem cair em recursao.
    _G.SXECloneTPRaw = doVelocityTP

    -- ===== GRABBLE TP (porte 1:1 do hub1 -- ciclo grapple -> Clone-TP) =====
    -- 1) acha a base alvo no cache do scanner
    -- 2) pede streaming da regiao (evita rollback do servidor)
    -- 3) dispara o grapple hasheado NA DIRECAO da base (RE/UseItem -> funciona
    --    igual no PC e no CELULAR, nao depende de teclado/mouse)
    -- 4) espera 0.12s (Activate e Deferred: com 0 o tapete tira a hook da mao)
    -- 5) roda o motor Clone-TP normal
    -- Usado por Auto TP, TP-on-load, Manual TP e Click Detect, porque todos
    -- passam por runAutoSnipe -> _fxTpDispatch.
    local function _sxeAcharDestino()
        local destino
        pcall(function()
            local plots = workspace:FindFirstChild("Plots")
            local cache = (_G.SharedState and _G.SharedState.AllAnimalsCache) or {}
            if not plots then return end
            for _, a in ipairs(cache) do
                if a.plot and a.owner ~= LP.Name and a.owner ~= LP.DisplayName then
                    local pl = plots:FindFirstChild(a.plot)
                    if pl then
                        local okp, pv = pcall(function() return pl:GetPivot().Position end)
                        if okp and pv then destino = pv; break end
    ...
    -- ============================================================
    --  GANCHO 1:1 DO jp.txt  (carpetEngage)
    -- ------------------------------------------------------------
    -- 1) espera ate 3s a Grapple Hook replicar na mochila E o _G.Net subir
    -- 2) equipa e ESPERA o equip registrar (nao usa delay fixo)
    -- 3) dispara o RE/UseItem hasheado real (sem argumento, como o jp)
    -- 4) desequipa e coloca o tapete, esperando ele aparecer na mao
    -- ============================================================
    local function _sxeAtivarGancho()
        local char = LP.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        ...
        local function _find(n)
            local bp = LP:FindFirstChild("Backpack")
            return (LP.Character and LP.Character:FindFirstChild(n)) or (bp and bp:FindFirstChild(n))
        end

        local _t0 = os.clock()
        while not _find("Grapple Hook") and os.clock() - _t0 < 3 do
            RunService.Heartbeat:Wait()
        end
        local g = _find("Grapple Hook")
        if g and g.Parent ~= LP.Character then
            pcall(function() hum:EquipTool(g) end)
            local _te = os.clock()
            while not (LP.Character and LP.Character:FindFirstChild("Grapple Hook"))
                  and os.clock() - _te < 0.5 do
        ...
        local fired = false
        if LP.Character and LP.Character:FindFirstChild("Grapple Hook") then
            if _G.SXEFireGrapple2 then
                pcall(function() fired = _G.SXEFireGrapple2() and true or false end)
            end
            if not fired then
                -- fallback: ativa a propria Tool (o script do jogo dispara o remote)
                local tool = LP.Character:FindFirstChild("Grapple Hook")
                if tool then fired = pcall(function() tool:Activate() end) end
                if not fired then
                    warn("[SXE] gancho: UseItem nao resolvido")
                end
            end
        else
            local _bp = LP:FindFirstChild("Backpack")
            warn(("[SXE] gancho pulado -- mochila=%s mao=%s remote=%s"):format(
                tostring(_bp ~= nil and _bp:FindFirstChild("Grapple Hook") ~= nil),
                tostring(LP.Character ~= nil and LP.Character:FindFirstChild("Grapple Hook") ~= nil),
                tostring(_G.SXEGrappleReady and _G.SXEGrappleReady() or false)))
        end

        -- tapete depois do puxao (igual jp: 0.08 / unequip / 0.08 / equipa tapete)
        task.wait(0.08)
        local h = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
        if h then pcall(function() h:UnequipTools() end) end
        task.wait(0.08)
        local _pref = (Config and Config.TpSettings and Config.TpSettings.Tool) or "Flying Carpet"
        local _tc = os.clock()
        repeat
            local c = LP.Character
            local hh = c and c:FindFirstChildOfClass("Humanoid")
            local carpet = _find(_pref)
            if not carpet then
                for _, cn in ipairs(CARPET_NAMES or {}) do
                    carpet = _find(cn)
                    if carpet then break end
                end
            end
            if carpet and hh then pcall(function() hh:EquipTool(carpet) end) end
            if carpet and c and carpet.Parent == c then break end
            RunService.Heartbeat:Wait()
        until os.clock() - _tc > 1
        return fired
    end
    _G.SXEAtivarGancho = _sxeAtivarGancho

    _G.SXEGrappleTP = function()
        if _G.__SXEGrappleRodando then return end
        _G.__SXEGrappleRodando = true
        local okAll, errAll = pcall(function()
            local destino = _sxeAcharDestino()
            -- TP FIX: antes TUDO isto estava dentro de `if destino then`. Se o
            -- scanner ainda nao tinha alvo no cache, o gancho nunca era equipado,
            -- o tapete nunca entrava na mao e o clique em Teleport parecia "nao
            -- fazer nada". Agora o gancho/tapete SEMPRE sao preparados; o destino
            -- so serve para pedir streaming e mirar o puxao.
            if destino and workspace.StreamingEnabled then
                pcall(function() LP:RequestStreamAroundAsync(destino) end)
            end
            -- GANCHO + TAPETE: sequencia portada do jp (equipa hook, dispara
            -- UseItem hasheado, desequipa, poe o tapete na mao).
            pcall(_sxeAtivarGancho)
            if false then
            local _char = LP.Character
            local _hum = _char and _char:FindFirstChildOfClass("Humanoid")
            if _hum then
                pcall(function() _hum:UnequipTools() end)
                local _pref = (Config and Config.TpSettings and Config.TpSettings.Tool) or "Flying Carpet"
                local _bp2 = LP:FindFirstChild("Backpack")
                local _carpet = (_char and _char:FindFirstChild(_pref)) or (_bp2 and _bp2:FindFirstChild(_pref))
                if not _carpet and _bp2 then
                    for _, cn in ipairs(CARPET_NAMES or {}) do
                        local c = _bp2:FindFirstChild(cn) or (_char and _char:FindFirstChild(cn))
                        if c then _carpet = c; break end
                    end
                end
                if _carpet then pcall(function() _hum:EquipTool(_carpet) end); task.wait(0.06) end
            end
            end
            -- Mesmo sem grapple (hook em cooldown / sem alvo no cache) o TP roda,
            -- entao o Grabble TP nunca fica "morto" -- so perde o puxao.
            doVelocityTP()
        end)
        _G.__SXEGrappleRodando = false
        if not okAll then warn("[SXE] Grabble TP:", errAll) end
    end
    ...
    -- the character spun. T is now handled ONLY by the "Manual TP" keybind, exactly like sxehub.)
...
-- =====================================================================
-- TP ENGINE SWAP -- sxehub's V1 "SXETPv1" engine transplanted below (replaces the
-- old doVelocityTP, which stays but is bypassed). These shims map the V1 engine's
-- _G.SXE_getNet / _G.SXEFireUseItem calls onto fixedsxe's _G.Net remote layer.
-- =====================================================================
_G.SXE_getNet = _G.SXE_getNet or function() return _G.Net end
_G.SXEFireUseItem = _G.SXEFireUseItem or function(...)
...
-- helper names never collide with dtc's V2 engine. Exposes _G.SXETPv1;
...
_G.SXEInstaReset = function()
    if _instaResetCooldown then return end
    if not _G.SXEResetRemote then return end
    _instaResetCooldown = true
    _G.SXESkipAutoTPUntil = os.clock() + 8   -- block the auto-TP chain after a manual reset
    local LP = game:GetService("Players").LocalPlayer
    local oldChar = LP.Character
    task.spawn(function()
        local startT = os.clock()
        while LP.Character == oldChar and os.clock() - startT < 5 do
            pcall(function() _G.SXEResetRemote:FireServer("randomstring") end)
            task.wait()
        end
...
_G.VanishGetSyncData = _G.VanishGetSyncData or function(plot)
    ...
    local ch = _G.SXE_GetPlotChannel and _G.SXE_GetPlotChannel(plotName) or nil
    if ch then
...
local NetModule
-- Single Net layer: source the Net module from the cat mapper's _G.SXE_getNet()
-- accessor (never require()s or scans Net directly).
local function loadNet()
    if NetModule then return true end
    NetModule = _G.SXE_getNet and _G.SXE_getNet() or nil
...
local function fireGrapple()
    ...
    -- METODO HERESY (hook-free): mira via PlayerMouse + Activate(), nunca fire cru
    do
        local ch = LP.Character
        local h2 = ch and ch:FindFirstChild("HumanoidRootPart")
        if h2 and _G.SXEFireGrapple2 then
            -- jp.txt: fire with default float value (0.33), NOT a Vector3
            pcall(_G.SXEFireGrapple2)
        end
    end
end
...
local function velMoveThrough(hrp, waypoints, speedOverride, allowJump, quickStart, rerouteLift)
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        if _G.SXETPCancel then cancelStop() return end
        equipCarpet()
        local target = waypoints[wpIdx]
        local diff = target - hrp.Position
        local mag = diff.Magnitude
    ...
    while not done and elapsed < timeout do
        task.wait(0.05)
        elapsed = elapsed + 0.05
        if _G.SXETPCancel then break end
        if not hrp or not hrp.Parent then break end
    ...
    if _G.SXETPCancel then cancelStop() else finish() end
    vZero(hrp)
end
...
local function doClone()
    ...
    local useOk = pcall(function() _G.SXEFireUseItem() end)
    task.wait(0.05)
...
local function doVelocityTP()
    ...
    if not _G.SXETPStartedAt then _G.SXETPStartedAt = os.clock() end  -- first TP start (admin panel waits on this)
    clearViz()
    if not NetModule then pcall(loadNet) end
    ...
    if #allPets == 0 then
        ...
        while #allPets == 0 and os.clock() - _t0 < 4 do
            if _G.SXETPCancel then isTeleporting = false; return end
            task.wait(0.15)
            allPets = scanAllPets()
        end
    end
    ...
    local pet = allPets[1]
    do
        local overrideUID = _G.SXEStealTargetUID
    ...
    local _animOn = _G.SXEAnimations == true
    local _animTok
    if _animOn then
        _animTok = (_G._tpAnimToken or 0) + 1
        _G._tpAnimToken = _animTok
        _G.SXETPForceNoAnims = true
        if _G.SXEKillAnims and LP.Character then pcall(_G.SXEKillAnims, LP.Character) end
        task.delay(30, function()   -- safety: never leave anims off forever
            if _G._tpAnimToken == _animTok then
                _G.SXETPForceNoAnims = false
                if _G.SXEAnimations and LP.Character and _G.SXERestoreAnims then pcall(_G.SXERestoreAnims, LP.Character) end
            end
        end)
    end
    ...
    local adjY = petPos.Y
    if TALL_PETS[petName] then adjY = petPos.Y - TALL_OFFSET end
    -- ApproachFloor2From1 (V1 SXETPv1 port, mirrors V2 clamp): force the floor-1
    ...
    if petPos.Y <= 8.9 and isPlotUnlocked(pet.plot) then
        ...
        healConn:Disconnect()
        isTeleporting = false
        if _G.SXETPCancel then _restoreAnims(); return end
        -- steal -- NO clone.
        pcall(function()
    ...
    _G.SXETPv1 = doVelocityTP
end

-- Route fixedsxe's TP through sxehub's V1 engine via a settings bridge (sxehub's _tpDispatch,
...
-- Highest/Priority/Nearest/manually-selected pet exactly like sxehub, not just allPets[1].
local function _fxTpDispatch()
    ...
    _G.SXEStealTargetUID = manuallySelectedUID and ((tostring(manuallySelectedUID):gsub("_", "|"))) or nil
    ...
    _G.SXETPCancel = false
    -- Grabble TP ON -> usa o motor de grapple/clone (metodo do jp).
    -- Vale para PC e mobile: o disparo do grapple e feito por remote.
    if Config.TpSettings and Config.TpSettings.GrabbleTP and _G.SXEGrappleTP then
        if _G.SXESetCarpetSpeed then
            pcall(_G.SXESetCarpetSpeed, tonumber(Config.TpSettings.GrabbleTPSpeed) or 230)
        end
        return _G.SXEGrappleTP()
    end
    if _G.SXETPv1 then return _G.SXETPv1() end
end
_G.SXEStartSideTP      = _fxTpDispatch
doGrabbleVelocityTP    = _fxTpDispatch
_G.SXE_ExecuteManualTP = function() task.spawn(function() pcall(_fxTpDispatch) end) end
-- AUTO CLOSE GUI
_G.setAutoCloseGUI = function(on)
    Config.AutoCloseOnExec = on
    saveConfig()
end

print("STL HUB EXECUTADO")
